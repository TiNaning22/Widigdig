...
function tambahKeranjangBarcode($data) {
    ...
    $keranjang_diskon_persen = isset($data['keranjang_diskon_persen']) ? $data['keranjang_diskon_persen'] : 0;
...
function tambahSupplier($data) {
	...
	$supplier_kode      = htmlspecialchars($data["supplier_kode"]);
	$supplier_nama      = htmlspecialchars($data["supplier_nama"]);
	$supplier_wa        = htmlspecialchars($data["supplier_wa"]);
	$supplier_bank       = htmlspecialchars($data["supplier_bank"]);
	$supplier_norekening = htmlspecialchars($data["supplier_norekening"]);
	$supplier_email     = htmlspecialchars($data["supplier_email"]);
	$supplier_alamat    = htmlspecialchars($data["supplier_alamat"]);
	$supplier_company   = htmlspecialchars($data["supplier_company"]);
	$supplier_kota      = htmlspecialchars($data["supplier_kota"]);
	$supplier_kodepos   = htmlspecialchars($data["supplier_kodepos"]);
	$supplier_status    = htmlspecialchars($data["supplier_status"]);
	$supplier_cabang    = htmlspecialchars($data["supplier_cabang"]);
	$supplier_create    = date("Y-m-d H:i:s");

	// Cek apakah WA sudah terdaftar
	$cek = mysqli_query($conn, "SELECT * FROM supplier WHERE supplier_wa = '$supplier_wa'");
	if (mysqli_num_rows($cek) > 0) {
		echo "<script>alert('No. WhatsApp Sudah Terdaftar');</script>";
		return 0;
	}

	// Simpan data
	$query = "INSERT INTO supplier (
		supplier_kode, supplier_nama, supplier_wa, supplier_email, 
		supplier_alamat, supplier_company, supplier_kota, supplier_kodepos,
		supplier_bank, supplier_norekening,
		supplier_status, supplier_create, supplier_cabang
	) VALUES (
		'$supplier_kode', '$supplier_nama', '$supplier_wa', '$supplier_email',
		'$supplier_alamat', '$supplier_company', '$supplier_kota', '$supplier_kodepos',
		'$supplier_bank', '$supplier_norekening',
...
function editSupplier($data) {
    global $conn;
    $id = (int)$data["supplier_id"]; // pastikan id adalah integer

    // Sanitasi input
    $supplier_kode        = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_kode"]));
    $supplier_nama        = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_nama"]));
    $supplier_wa          = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_wa"]));
    $supplier_bank        = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_bank"]));
    $supplier_norekening  = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_norekening"]));
    $supplier_email       = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_email"]));
    $supplier_alamat      = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_alamat"]));
    $supplier_company     = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_company"]));
    $supplier_kota        = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_kota"]));
    $supplier_kodepos     = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_kodepos"]));
    $supplier_status      = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_status"]));
    $supplier_cabang      = mysqli_real_escape_string($conn, htmlspecialchars($data["supplier_cabang"]));
    $supplier_update      = date("Y-m-d H:i:s");
    ...
    $query = "UPDATE supplier SET 
        ...
        supplier_norekening  = '$supplier_norekening',
        ...
        supplier_cabang      = '$supplier_cabang',
        updated_at           = '$supplier_update'
...
// ===================================== Keranjang Pembelian =============================== //
function tambahKeranjangPembelian($barang_id, $keranjang_nama, $keranjang_harga, $keranjang_id_kasir, $keranjang_qty, $keranjang_cabang, $keranjang_id_cek, $keranjang_diskon = 0) {
    global $conn;
    ...
    if ( $barang_id != null ) {
        if ( $barang_id_cek > 0 ) {
            ...
            $query = "UPDATE keranjang_pembelian SET 
                        keranjang_qty   = '$kp',
                        keranjang_diskon = '$keranjang_diskon'
                        WHERE keranjang_id_cek = $keranjang_id_cek
                        ";
            mysqli_query($conn, $query);
            return mysqli_affected_rows($conn);
            ...
            // query insert data
            $query = "INSERT INTO keranjang_pembelian VALUES ('', '$keranjang_nama', '$keranjang_harga', '$barang_id', '$keranjang_qty', '$keranjang_id_kasir', '$keranjang_id_cek', '$keranjang_cabang', '$keranjang_diskon')";
