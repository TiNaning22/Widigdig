...
if( isset($_POST["submit"]) ){
  ...
  if( tambahBarangInternal($_POST) > 0 ) {
    echo "
      <script>
        document.location.href = 'penerimaan-barang-internal';
      </script>
    ";
  } else {
    echo "
...
// Fungsi untuk menambahkan data ke tabel barang_internal
function tambahBarangInternal($data) {
  global $conn;
  ...
  $satuan_id = htmlspecialchars($data['satuan_id']);
  $satuan_id_2 = htmlspecialchars($data['satuan_id_2']);
  $satuan_id_3 = htmlspecialchars($data['satuan_id_3']);
  $satuan_isi_2 = htmlspecialchars($data['satuan_isi_2']);
  $satuan_isi_3 = htmlspecialchars($data['satuan_isi_3']);
  ...
  $barang_cabang = htmlspecialchars($data['barang_cabang']);
  $barang_kode_count = htmlspecialchars($data['barang_kode_count']);
  $barang_option_sn = htmlspecialchars($data['barang_option_sn']);

  // Query untuk menambahkan data ke tabel barang_internal
  $query = "INSERT INTO barang_internal (
    barang_kode, barang_kode_slug, barang_kode_count, barang_nama, barang_harga_beli, barang_harga, barang_harga_grosir_1, barang_harga_grosir_2, barang_harga_s2, barang_harga_grosir_1_s2, barang_harga_grosir_2_s2, barang_harga_s3, barang_harga_grosir_1_s3, barang_harga_grosir_2_s3, barang_stock, barang_tanggal, barang_kategori_id, kategori_id, barang_satuan_id, satuan_id, satuan_id_2, satuan_id_3, satuan_isi_1, satuan_isi_2, satuan_isi_3, barang_deskripsi, barang_option_sn, barang_penyimpanan, barang_kadaluarsa, barang_no_batch, barang_terjual, barang_cabang
  ) VALUES (
    '$barang_kode', '', '$barang_kode_count', '$barang_nama', '0', '$barang_harga', '$barang_harga_grosir_1', '$barang_harga_grosir_2', '$barang_harga_s2', '$barang_harga_grosir_1_s2', '$barang_harga_grosir_2_s2', '$barang_harga_s3', '$barang_harga_grosir_1_s3', '$barang_harga_grosir_2_s3', '$barang_stock', NOW(), '$kategori_id', '$kategori_id', '$satuan_id', '$satuan_id', '$satuan_id_2', '$satuan_id_3', '1', '$satuan_isi_2', '$satuan_isi_3', '$barang_deskripsi', '$barang_option_sn', '$barang_penyimpanan', '$barang_kadaluarsa', '$barang_no_batch', '0', '$barang_cabang'
  )";
...
?>
  ...
  <?php 
      $barang = mysqli_query($conn,"select * from barang_internal where barang_cabang = ".$sessionCabang." ");
      $jmlBarang = mysqli_num_rows($barang); 

      if ( $jmlBarang < 1 ) {
          $barangCount = 1;
      } else {
          $barangCount = query("SELECT * FROM barang_internal ORDER BY barang_id DESC LIMIT 1")[0];
